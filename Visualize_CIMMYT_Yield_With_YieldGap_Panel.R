...
# =============================================================================
# Panel A:
#   Annual observed mean ± SD and simulated MME mean with transparent SD ribbons
#   plus regression lines for:
#     1980-1998
#     1998-2019
#     whole period
#
# Panel B:
#   Annual yield gap = simulated MME mean - observed CIMMYT mean
#   Positive value means model overestimates observed yield.
#   Negative value means model underestimates observed yield.
#
# Outputs:
#   Yield_AnnualMean_Observed_Simulated_With_YieldGap.png
#   Yield_AnnualMean_Observed_Simulated_With_YieldGap.pdf
#   Yield_AnnualMean_Observed_Simulated_With_YieldGap.csv
#   Yield_Regression_Slopes_Observed_Simulated.csv
# =============================================================================
...
plot_df <- full_join(obs_year, sim_year, by = "Year") %>%
  ...
  mutate(
    ...
    gap_type = ifelse(yield_gap >= 0, "Simulated > Observed", "Simulated < Observed")
  )
...
# -----------------------------------------------------------------------------
# 6) PANEL A: OBSERVED VS SIMULATED
# -----------------------------------------------------------------------------
...
combined_plot <- p_a / p_b +
  ...
  plot_annotation(
    ...
    subtitle = "Panel A shows annual yield means, uncertainty, and segmented regressions. Panel B shows yearly model bias as simulated minus observed yield.",
    theme = theme(
...
ggsave(
  filename = file.path(output_dir, "Yield_AnnualMean_Observed_Simulated_With_YieldGap.png"),
  plot = combined_plot,
  width = fig_width,
  height = fig_height,
  dpi = 300
...
ggsave(
  filename = file.path(output_dir, "Yield_AnnualMean_Observed_Simulated_With_YieldGap.pdf"),
  plot = combined_plot,
  width = fig_width,
  height = fig_height
)
