...
# + enlarged figure and readable legends
# =============================================================================
...
p_b <- ggplot(plot_df, aes(x = Year, y = yield_gap)) +
  ...
  geom_point(color = "black", size = 2.4, na.rm = TRUE) +
  scale_x_continuous(
    breaks = x_breaks,
    minor_breaks = seq(min(x_breaks), max(x_breaks), by = 1)
  ) +
...
ggsave(
  filename = file.path(output_dir, "Yield_AnnualMean_Observed_Simulated_With_YieldGap_RegStats_LargeLegend.png"),
  plot = combined_plot,
  width = fig_width,
  height = fig_height,
  dpi = 300
...
ggsave(
  filename = file.path(output_dir, "Yield_AnnualMean_Observed_Simulated_With_YieldGap_RegStats_LargeLegend.pdf"),
  plot = combined_plot,
  width = fig_width,
  height = fig_height
)
