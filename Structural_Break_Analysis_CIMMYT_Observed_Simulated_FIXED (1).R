...
# FIXED VERSION: uses dplyr::select() explicitly because MASS::select masks dplyr::select()
# =============================================================================
...
obs <- obs_raw %>%
  ...
  dplyr::filter(!is.na(Year), !is.na(Yield_obs))
...
sim <- sim_raw %>%
  ...
  dplyr::filter(!is.na(Year), !is.na(Yield_sim))

obs_year <- obs %>%
  dplyr::group_by(Year) %>%
  dplyr::summarise(
    obs_mean = mean(Yield_obs, na.rm = TRUE),
    obs_sd = ifelse(dplyr::n() > 1, sd(Yield_obs, na.rm = TRUE), NA_real_),
    obs_n = dplyr::n(),
    .groups = "drop"
  )
...
sim_year <- sim %>%
  ...
  dplyr::summarise(
    ...
    sim_sd = ifelse(dplyr::n() > 1, sd(Yield_sim, na.rm = TRUE), NA_real_),
    sim_n = dplyr::n(),
    .groups = "drop"
  )

annual <- dplyr::full_join(obs_year, sim_year, by = "Year") %>%
  dplyr::arrange(Year) %>%
  dplyr::mutate(yield_gap = sim_mean - obs_mean)
...
safe_segmented <- function(df, y_col, series_name, psi_guess = 1998) {
  ...
  dat <- df %>%
    ...
    dplyr::select(Year, y = dplyr::all_of(y_col))
  ...
  fit <- tryCatch(
    segmented::segmented(lm0, seg.Z = ~Year, psi = psi_guess),
    error = function(e) {
      message("Segmented failed for ", series_name, ": ", conditionMessage(e))
      NULL
    }
  )
...
safe_breakpoints <- function(df, y_col, series_name) {
  ...
  bp <- tryCatch(
    strucchange::breakpoints(y ~ Year, data = dat, h = 0.15),
    error = function(e) {
      message("Breakpoints failed for ", series_name, ": ", conditionMessage(e))
      NULL
    }
  )
  ...
  bp_best <- strucchange::breakpoints(y ~ Year, data = dat, breaks = best_k, h = 0.15)
...
manual_chow_test <- function(df, y_col, series_name, break_year = 1998) {
  ...
  before <- dat %>% dplyr::filter(Year <= break_year)
  after  <- dat %>% dplyr::filter(Year > break_year)
...
segmented_results <- dplyr::bind_rows(seg_obs$result, seg_sim$result) %>%
  dplyr::mutate(
...
breakpoint_results <- dplyr::bind_rows(
...
period_results <- dplyr::bind_rows(
...
seg_pred <- dplyr::bind_rows(seg_obs$pred, seg_sim$pred)

plot_long <- annual %>%
  dplyr::select(Year, obs_mean, sim_mean) %>%
  tidyr::pivot_longer(
...
p1 <- ggplot() +
  ...
  geom_line(
    data = plot_long %>% dplyr::filter(Series == "Simulated MME"),
