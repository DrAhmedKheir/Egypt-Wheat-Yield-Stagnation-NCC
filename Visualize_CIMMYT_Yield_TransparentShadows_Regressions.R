# =============================================================================
# CIMMYT annual mean observed vs simulated yield with transparent SD ribbons
# + 2-year x-axis ticks
# + segmented regression lines for observed and simulated
# =============================================================================
...
write.csv(
  plot_df,
  file.path(output_dir, "Yield_AnnualMean_Observed_vs_Simulated_TransparentShadows_Regressions.csv"),
  row.names = FALSE
)
...
make_regression_segment <- function(df, y_col, series_name, period_name, start_year, end_year) {
  ...
  if (nrow(tmp) < 2) {
    return(list(
      pred = tibble(),
      stats = tibble(
        Series = series_name, Period = period_name,
        StartYear = start_year, EndYear = end_year,
        Slope_t_ha_per_year = NA_real_, Intercept = NA_real_,
        R2 = NA_real_, P_value = NA_real_, N = nrow(tmp)
      )
    ))
  }
...
p <- ggplot(plot_df, aes(x = Year)) +
  ...
  labs(
    title = "Annual mean observed CIMMYT yield vs simulated MME yield",
    subtitle = "Observed dots = annual mean; error bars = SD when multiple observations exist. Simulated line = MME mean; transparent shadows = SD across locations and models. Regression lines are shown for 1980-1998, 1998-2019, and the whole period.",
    x = "Year",
    y = "Grain yield (t/ha)"
  ) +
...
ggsave(
  filename = file.path(output_dir, "Yield_AnnualMean_Observed_vs_Simulated_TransparentShadows_Regressions.png"),
  plot = p,
  width = fig_width,
  height = fig_height,
  dpi = 300
...
ggsave(
  filename = file.path(output_dir, "Yield_AnnualMean_Observed_vs_Simulated_TransparentShadows_Regressions.pdf"),
  plot = p,
  width = fig_width,
  height = fig_height
)
