...
calculate_regression <- function(data, year, value){
...
# =============================================================================
# Function to convert P values to publication format
# =============================================================================
...
tmp <- calculate_regression(df,
                            df$Year,
                            df$Mean_Tmax_SowToAnth)

tmp$Variable <- "Tmax"
tmp$Stage <- "Sowing-Anthesis"

regression_statistics <-
  rbind(regression_statistics,tmp)

tmp <- calculate_regression(df,
                            df$Year,
                            df$Mean_Tmax_AnthToMat)

tmp$Variable <- "Tmax"
tmp$Stage <- "Anthesis-Maturity"
...
regression_statistics$Intercept <-
  round(regression_statistics$Intercept,3)

regression_statistics$Slope_per_year <-
  round(regression_statistics$Slope_per_year,3)

regression_statistics$Slope_per_decade <-
  round(regression_statistics$Slope_per_decade,3)

regression_statistics$R2 <-
  round(regression_statistics$R2,3)

regression_statistics$P_text <-
  sapply(regression_statistics$P,format_p)
...
write.csv(
  ...
  file.path(
    outdir,
    "Figure2_RegressionStatistics.csv"
  ),
...
for(i in 1:nrow(regression_statistics)){
  
  cat(
    
    paste0(
      
      regression_statistics$Variable[i],
      
      " (",
      
      regression_statistics$Stage[i],
      ...
      sprintf("%.2f",
              ...
              regression_statistics$Slope_per_decade[i]),
      ...
      sprintf("%.2f",
              ...
              regression_statistics$R2[i]),
      
      "; P = ",
      
      regression_statistics$P_text[i],
...
cat("\n")
cat("===============================\n")
cat("Figure 2 regression statistics\n")
cat("===============================\n\n")

print(regression_statistics)